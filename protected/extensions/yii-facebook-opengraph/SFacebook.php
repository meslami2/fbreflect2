...
class SFacebook extends CApplicationComponent
{
  ...
  */
  ...
  public function getLinkedProfilePicture($id) {
        if ($this->getUser()) {
            $profile = $this->getInfoById($id);
            $url = $profile["link"];
            $name = $profile["name"];
            $image = $this->getProfilePictureById($id);
            return "<a href=\"" . "#" . "\"><img alt=\"" . $name . "\" src=\"" . $image . "\"></a>";
        } else {
            return false;
        }
    }
    ...
    public function getFullName($id) {
        //return $id . "'s Name";
        //$fullname = $this->_getFacebook()->api_client->users_getInfo($id, "name");
        $userinfo = $this->getInfoById($id);
